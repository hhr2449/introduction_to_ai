...
class Corpus3G:
    ...
    def __init__(self, corpus_path, pinyin_path, valid_char_table_path):
        # 关键的数据表：拼音汉字对照表，合法汉字表，单字频数表，双字频数表，三字频数表
        self.pinyin_table, self.valid_char_table = self.load_pinyin_table(pinyin_path, valid_char_table_path)
        self.uniword_table, self.bigram_table, self.trigram_table = self.calculate_frequency_corpus(corpus_path)
        # 统计一元字符总频数，二元字符总频数，三元字符总频数，出现过的字符总类数
        self.uni_count = sum(self.uniword_table.values())
        self.bi_count = sum(self.bigram_table.values())
        self.tri_count = sum(self.trigram_table.values())
                    ...
                    for line in all_lines_list:
                        for char in line:
                            if char in self.valid_char_table:
                                ...
                                # 统计二元
                                if prev1 != None:
                                    char_pair = (prev1, char)
                                    if char_pair not in bigram_table:
                                        bigram_table[char_pair] = 0
                                    bigram_table[char_pair] += 1
                                # 统计三元
                                if prev1 != None and prev2 != None:
                                    char_triple = (prev2, prev1, char)
                                    if char_triple not in trigram_table:
                                        trigram_table[char_triple] = 0
                                    trigram_table[char_triple] += 1
                                ...
                                prev1 = None
                                prev2 = None
        return uniword_table, bigram_table, trigram_table
