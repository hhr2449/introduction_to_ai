'''
model.py
ProbCalculator用于计算概率，运行viterbi算法
ParserPinyin用于读取拼音并且进行解码
'''
...
class ProbCalculator:
    ...
    def get_unigram(self, word):
        ...
        self.uniword_cache[word] = -math.log((count + self.alpha) / (total + self.alpha * num_of_types))
        return self.uniword_cache[word]
    ...
    def get_bigram(self, word1, word2):
        ...
        # 2. 获取 P(w2) 的概率 
        p_w2 = (self.corpus.uniword_table.get(word2, 0) + self.alpha) / \
            (self.corpus.uni_count + self.alpha * self.corpus.num_of_types)
...
class ParserPinyin:
    def __init__(self, corpus, lam=0.89, alpha=0.1):
        self.corpus = corpus
        self.calculator = ProbCalculator(corpus, lam=lam, alpha=alpha)
    # 解析拼音列表
    def parser_pinyin_list(self, pinyin_list):
        return "".join(viterbi(pinyin_list, self.corpus, self.calculator))
